    ...
    function onClickCopyButton(e) {
        ...
        setSurveyList((prev) => {
            const duplicatedProblem = {
                ...
                problemType: prev.content[index].problemType,
                problemTitle: prev.content[index].problemTitle,
            ...
            <div className={styles.sidebar} >
                <div className={styles.content}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                        {list.map((name, index) => {
                            return <button key={index} className={problem && index === problem.problemType ? styles.setBtn_selected : styles.setBtn} style={{ width: "30%" }} onClick={(e) => { onClickTypeButton(e, index) }}>{name}</button>
                        })}
                    </div>
